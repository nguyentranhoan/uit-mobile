...
class BaseDatabase(ABC):
    ...
    def get_engine(self) -> Engine:
        if self.__engine is None:
            self.__engine = create_engine(self.get_db_url, echo=self.__show_sql)
        return self.__engine

    @property
    def get_session_maker(self) -> sessionmaker:
        if self.__sessionmaker is None:
            self.__sessionmaker = sessionmaker(autocommit=False, bind=self.get_engine, expire_on_commit=False)
