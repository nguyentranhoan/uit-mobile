...
def bad_request_exception_handler(_, exc: BadRequestException):
    response = HTTPExceptionResponse(exc.get_code(), exc.get_message(), errors=[])

    return JSONResponse(status_code=HTTPStatus.BAD_REQUEST, content=response.__dict__)
...
def request_validation_error_handler(_, exc: RequestValidationError):
    ...
    def loc_to_field(loc):
        lst: list = list(map(lambda e: str(e), list(loc)[1:]))
        if lst[0] is 'request':
            lst = lst[1:]
        return ','.join(lst)
